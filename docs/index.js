...
<button title="再生" class="btn p-0"><i class="bi bi-play-fill"></i></button>
  `}function play(a,b){if(!a){const c=$table.bootstrapTable("getData",{useCurrentPage:!0});a=document.querySelector("#midiList td:nth-child(2) i"),b=c[0]}const c=$table[0].querySelectorAll("tbody .bi-pause-fill, .bi-play");[...c].forEach(a=>{a.className="bi bi-play-fill"}),player.isPlaying()&&player.stop(),a.className="bi bi-pause-fill";const d=`${midiDB}/${b.file}`;loadMIDI(d).then(()=>{playMIDI(0)})}function replay(a){if(!a){const b="#midiList td:nth-child(2) i[class='bi bi-play']";a=document.querySelector(b)}if(a.className="bi bi-pause-fill",configChanged){player.stop();const a=parseInt(document.getElementById("seekbar").value),b=document.getElementById("speed"),c=b.value/100;playMIDI(a/c),configChanged=!1}else{player.resume();const a=parseInt(document.getElementById("seekbar").value);setSeekbarInterval(a)}}function pause(a){if(!a){const b="#midiList td:nth-child(2) i[class='bi bi-pause-fill']";a=document.querySelector(b)}a.className="bi bi-play",player.pause(),clearInterval(seekbarInterval)}window.toolEvents={"click .bi-play-fill":function(a,c,b,d){switch(a.target.className){case"bi bi-play-fill":return play(a.target,b);case"bi bi-play":return replay(a.target);case"bi bi-pause-fill":return pause(a.target)}}};function getInstrumentsString(a,b){const c=b.instruments.split(",").map(a=>parseInt(a));return c.map(b=>a[b]).join(", ")}function typeEvent(a){switch(a.code){case"Space":switch(a.preventDefault(),player.getPlayState()){case"paused":return replay();case"started":return pause();case"stopped":return play()}break}}function initFilterStates(){const a=new Map,b=document.querySelectorAll("#midiList > thead > tr > th");return[...b].slice(1).forEach(c=>{const b=c.dataset.field;console.log(b),a.set(b,"")}),a}loadConfig();const midiDB="https://midi-db.pages.dev",$table=$("#midiList"),soundFont="https://storage.googleapis.com/magentadata/js/soundfonts/sgm_plus",playerCallback={run:()=>{},stop:()=>{playNext()}},player=new core.SoundFontPlayer(soundFont,void 0,void 0,void 0,playerCallback),filterStates=initFilterStates();let ns,nsCache,configChanged=!1,seekbarInterval;const insturmentsPromise=loadInstruments();fetch(`${midiDB}/${document.documentElement.lang}.json`).then(a=>a.json()).then(a=>{insturmentsPromise.then(b=>{a.forEach(a=>{a.instruments=getInstrumentsString(b,a)})}),$table.bootstrapTable("load",a),[...document.querySelectorAll(".buttons-toolbar input")].forEach(a=>{a.addEventListener("change",addFilterControl)}),addFilterControl()}),document.getElementById("toggleDarkMode").onclick=toggleDarkMode,document.getElementById("lang").onchange=changeLang,document.getElementById("speed").onchange=changeSpeed,document.getElementById("speedDown").onclick=speedDown,document.getElementById("speedUp").onclick=speedUp,document.getElementById("repeat").onclick=repeat,document.getElementById("volumeOnOff").onclick=volumeOnOff,document.getElementById("volumebar").onchange=changeVolumebar,document.getElementById("seekbar").onchange=changeSeekbar,document.getElementById("instruments").onchange=changeInstruments,document.addEventListener("keydown",typeEvent),document.addEventListener("click",unlockAudio,{once:!0,useCapture:!0})