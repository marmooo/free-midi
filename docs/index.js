...
<button title="再生" class="btn p-0"><i class="bi bi-play-fill"></i></button>
  `}window.toolEvents={"click .bi-play-fill":function(a,d,b,c){switch(a.target.className){case"bi bi-play-fill":{const d=$table[0].querySelectorAll("tbody .bi-pause-fill, .bi-play");[...d].forEach(a=>{a.className="bi bi-play-fill"}),player.isPlaying()&&player.stop(),a.target.className="bi bi-pause-fill";const e=`${midiDB}/${b.file}`;playMIDI(a,c,e);break}case"bi bi-play":{a.target.className="bi bi-pause-fill",player.resume();const b=parseInt(document.getElementById("seekbar").value);setSeekbarInterval(b);break}case"bi bi-pause-fill":a.target.className="bi bi-play",player.pause(),clearInterval(seekbarInterval);break}}};function getInstrumentsString(a,b){const c=b.instruments.split(",").map(a=>parseInt(a));return c.map(b=>a[b]).join(", ")}const insturmentsPromise=loadInstruments();$("#midiList").bootstrapTable({onLoadSuccess:function(a){insturmentsPromise.then(b=>{a.forEach(a=>{a.instruments=getInstrumentsString(b,a)})})}}),document.getElementById("toggleDarkMode").onclick=toggleDarkMode,document.getElementById("lang").onchange=changeLang,document.getElementById("speedDown").onclick=speedDown,document.getElementById("speedUp").onclick=speedUp,document.getElementById("repeat").onclick=repeat,document.getElementById("volumeOnOff").onclick=volumeOnOff,document.getElementById("volumebar").onchange=changeVolumebar,document.getElementById("seekbar").onchange=changeSeekbar,document.getElementById("instruments").onchange=changeInstruments