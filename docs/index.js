...
<div class="d-flex overflow-scroll p-2">
  ...
  `}function _detailFormatterJa(i,a){const h=`${midiDB}/${a.file}`,e=toURLSearchParams(a),b=e.toString(),f=toLicense(a.license,a.file),d=toWeb(a.file),g=toDownload(a.id,h,"ja");let c="";return a.instruments.split(", ").forEach(a=>{c+=`<li>${a}</li>`}),`
<div class="d-flex overflow-scroll p-2">
  <div>
    <h5>楽譜</h5>
    <table class="table table-sm table-striped w-auto">
...
<button title="再生" class="btn p-0"><i class="bi bi-play-fill"></i></button>
  `}function play(a,b){if(!a){const c=$table.bootstrapTable("getData",{useCurrentPage:!0});a=document.querySelector("#midiList td:nth-child(2) i"),b=c[0]}const c=$table[0].querySelectorAll("tbody .bi-pause-fill, .bi-play");[...c].forEach(a=>{a.className="bi bi-play-fill"}),player.isPlaying()&&player.stop(),a.className="bi bi-pause-fill";const d=`${midiDB}/${b.file}`;loadMIDI(d).then(()=>{playMIDI(0)})}function replay(a){if(!a){const b="#midiList td:nth-child(2) i[class='bi bi-play']";a=document.querySelector(b)}if(a.className="bi bi-pause-fill",configChanged){player.stop();const a=parseInt(document.getElementById("seekbar").value),b=document.getElementById("speed"),c=b.value/100;playMIDI(a/c),configChanged=!1}else player.resume(currentTime),setTimer(currentTime)}function pause(a){if(!a){const b="#midiList td:nth-child(2) i[class='bi bi-pause-fill']";a=document.querySelector(b)}a.className="bi bi-play",player.pause(),clearInterval(timer)}window.toolEvents={"click .bi-play-fill":function(a,c,b,d){switch(a.target.className){case"bi bi-play-fill":return play(a.target,b);case"bi bi-play":return replay(a.target);case"bi bi-pause-fill":return pause(a.target)}}};function getInstrumentsString(a,b){const c=b.instruments.split(",").map(a=>parseInt(a));return c.map(b=>a[b]).join(", ")}function typeEvent(a){if(!player)return;if(controllerDisabled)return;switch(player.resumeContext(),a.code){case"Space":switch(a.preventDefault(),player.getPlayState()){case"paused":return replay();case"started":return pause();case"stopped":return play()}break}}function initFilterTexts(){const a=new Map,b=document.querySelectorAll("#midiList > thead > tr > th");return[...b].slice(1).forEach(b=>{const c=b.dataset.field;a.set(c,"")}),a}async function fetchCollections(){const a=await fetch(`${midiDB}/collections.json`);return await a.json()}function shuffle(a){for(let b=a.length;1<b;b--){const c=Math.floor(Math.random()*b);[a[c],a[b-1]]=[a[b-1],a[c]]}return a}async function fetchPlayList(d){const a=d.map(a=>a.id);shuffle(a);const b=document.documentElement.lang,e=await fetch(`${midiDB}/json/${a[0]}/${b}.json`),c=await e.json();$table.bootstrapTable("load",c),addFilterControl(),instrumentListPromise.then(a=>{c.forEach(b=>{b.instruments=getInstrumentsString(a,b)})});const f=a.slice(1).map(async a=>{const c=await fetch(`${midiDB}/json/${a}/${b}.json`);return c.json()});Promise.all(f).then(a=>{a.forEach(a=>{$table.bootstrapTable("append",a),addFilterControl();const b=document.querySelector(".buttons-toolbar");[...b.querySelectorAll("input")].forEach(a=>{a.addEventListener("change",addFilterControl)});const c=b.children[1].querySelector("button");c.addEventListener("click",()=>{$table.bootstrapTable("filterBy",{},{filterAlgorithm:()=>!0})}),instrumentListPromise.then(b=>{a.forEach(a=>{a.instruments=getInstrumentsString(b,a)})})})})}function addCollectionSelector(){const a=document.getElementById("collections");collections.forEach(c=>{const b=document.createElement("button");c.status=="CLOSED"?b.className="btn btn-sm btn-outline-secondary m-1":b.className="btn btn-sm btn-outline-primary m-1",b.textContent=c.name,b.onclick=()=>{const a=document.getElementById("midiList").querySelector("thead > tr > th[data-field='id'] input");a&&(a.value=c.id),filterTable("id",c.id)},a.appendChild(b)})}loadConfig();const midiDB="https://midi-db.pages.dev",$table=$("#midiList"),filterTexts=initFilterTexts(),collections=new Map;let controllerDisabled,currentTime=0,ns,nsCache,configChanged=!1,timer,player;const instrumentListPromise=loadInstrumentList();loadSoundFontList(),initPlayer(),fetchCollections().then(a=>{a.forEach(a=>{collections.set(a.id,a)}),addCollectionSelector(),fetchPlayList(a)}),document.getElementById("toggleDarkMode").onclick=toggleDarkMode,document.getElementById("lang").onchange=changeLang,document.getElementById("speed").onchange=changeSpeedEvent,document.getElementById("speedDown").onclick=speedDown,document.getElementById("speedUp").onclick=speedUp,document.getElementById("repeat").onclick=repeat,document.getElementById("volumeOnOff").onclick=volumeOnOff,document.getElementById("volumebar").onchange=changeVolumebar,document.getElementById("seekbar").onchange=changeSeekbar,document.getElementById("instruments").onchange=changeConfig,document.getElementById("soundfonts").onchange=changeConfig,document.getElementById("inputSoundFontFile").onchange=loadSoundFontFileEvent,document.getElementById("inputSoundFontUrl").onchange=loadSoundFontUrlEvent,document.addEventListener("keydown",typeEvent),document.addEventListener("click",unlockAudio)